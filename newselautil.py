...
def cleanSentences(slist):
    ...
    for s in slist:
        s1 = cleanSentence(s)
        if s1 == '' or s1.isspace(): continue # skip
        sentlist.append(s1)
    return sentlist


CAPITALIZE_WORDS = ['A', '``', 'U.S.', 'C.J.', 'U.K.', 'FBI', 'FDA', '###', '_', 'NASA', 'DNA', 'TV','UFO','B.B.','!\n']
# the list of capitalized words that are not locations name. This is used in modify_the_first_lines
...
# header lines from the newsela corpus that are not modified and left as they are


def modify_the_header(line):
    """
    delete the not useful headers such as (SEATTLE, Wash. --)
    ...
    if line not in LINES_TO_IGNORE:
        ...
        for letter in firstWord:
            if letter != letter.upper():
                isUpperCase = False
        if isUpperCase and firstWord not in CAPITALIZE_WORDS:
            i = 0
...
def getTokParagraphs(article, separateBySemicolon=True, MODIFY_HEADER = True):
    ...
    :param article:
    :param separateBySemicolon: if True, the parts of one sentence separated by a semicolon will be considered as
    separate sentences
    :return:
    """
    SUFFIX = ".tok"
    ...
    with io.open(path.BASEDIR + '/articles/' + article['filename']+SUFFIX,mode='r',encoding='utf-8') as fd:
        ...
        for i in range(len(lines)):
            if separateBySemicolon:
                ...
                if lines[i][0:len(PARPREFIX)] == PARPREFIX:  # without considering ";" to be a delimeter
                    cleaned = cleanSentences(slist)
                    if len(cleaned) > 0:
                        pars.append(cleaned)
                        slist = []
...
def lemmatize(s):
    ...
    #tokens = removeCaps(tokens) # remove any string with uppercase char (eg, proper names)
    cleantokens = [ ]
    for w in tokens:
        try:
            w_asc = w.encode('ascii')
            if (w not in string.punctuation): cleantokens.append( w ) 
